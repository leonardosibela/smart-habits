...
class DailyTasksFragment : Fragment() {
    ...
    private lateinit var periodicTaskAdapter: PeriodicTaskAdapter<DailyTask>

    private val viewModel: DailyTasksViewModel by viewModel()
    ...
    private fun onAddTaskClicked(view: View) {
        findNavController().navigate(
            DailyTasksFragmentDirections.actionDailyTasksFragmentToAddPeriodicTaskFragment(
                Periodicity.DAILY
            )
        )
    }
    ...
    private fun onTasksChanged(taskResult: PeriodicTaskResult<DailyTask>) {
        when (taskResult) {
    ...
    private fun displayTasks(data: List<DailyTask>) = with(binding) {
        dailyTasksError.isVisible = false
