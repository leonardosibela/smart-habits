...
import com.sibela.smarthabits.presentation.viewmodel.MonthlyTasksViewModel
import com.sibela.smarthabits.presentation.viewmodel.PeriodicTaskResult
import org.koin.androidx.viewmodel.ext.android.viewModel

class MonthlyTasksFragment : Fragment() {

    private lateinit var periodicTaskAdapter: PeriodicTaskAdapter<MonthlyTask>

    private val viewModel: MonthlyTasksViewModel by viewModel()
    ...
    private fun onAddTaskClicked(view: View) {
        findNavController().navigate(
            MonthlyTasksFragmentDirections.actionMonthlyTasksFragmentToAddPeriodicTaskFragment(
                Periodicity.MONTHLY
            )
        )
    }
    ...
    private fun onTasksChanged(taskResult: PeriodicTaskResult<MonthlyTask>) {
        when (taskResult) {
    ...
    private fun displayTasks(data: List<MonthlyTask>) = with(binding) {
        monthlyTasksError.isVisible = false
    ...
    private fun deleteTask(task: MonthlyTask) {
